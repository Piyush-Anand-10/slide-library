...
export const PresentationViewer = ({ presentation, onClose }: Props) => {
  ...
  const hasFile = Boolean(presentation.fileUrl);
  const fileAbsoluteUrl = hasFile
    ? `${window.location.origin}${presentation.fileUrl}`
    : null;
  const viewerUrl = fileAbsoluteUrl
    ? `https://docs.google.com/viewer?url=${encodeURIComponent(fileAbsoluteUrl)}&embedded=true`
    : null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/60 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      >
        ...
        {/* Controls — only shown for placeholder (no real file) */}
        {!hasFile && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-border">
            ...
            <div className="flex items-center gap-1.5">
              {Array.from({ length: Math.min(presentation.slideCount, 10) }, (_, i) => {
                const slideNum = Math.min(Math.max(currentSlide - 4, 1) + i, presentation.slideCount);
                return (
                  <button
                    key={i}
